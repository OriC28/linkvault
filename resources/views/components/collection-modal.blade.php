<div
    ...
    @collection-modal.window="setup($event.detail)"
...
>
    ...
    <div x-show="open" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
        ...
        <div
            ...
            <form :action="isEdit ? `/collections/show/${slug}` : '{{ route('collections.store') }}'" method="POST">
                @csrf
                ...
                <div class="px-6 py-4 border-b border-[#d2d2d7]/60 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-[#1d1d1f]" x-text="isEdit ? 'Editar colección' : 'Nueva colección'"></h3>
                    <button type="button" @click="open = false" class="text-[#86868b] hover:text-[#1d1d1f] p-1 rounded-lg">
                        ✕
                    </button>
                </div>
                ...
                <div class="p-6 space-y-4">
                    <!-- Nombre -->
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-[#86868b] mb-1">Nombre</label>
                        <input
                            type="text"
                            name="name"
                            x-model="name"
                    ...
                    <!-- Descripción -->
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-[#86868b] mb-1">Descripción</label>
                        <textarea
                            name="description"
                            x-model="description"
                            rows="3"
                            placeholder="Descripción (opcional)"
                            class="w-full bg-white/70 rounded-xl border border-[#d2d2d7] px-4 py-2.5 text-sm text-[#1d1d1f] focus:ring-2 focus:ring-[#007AFF]/30 focus:border-[#007AFF] outline-none resize-none"
                        ></textarea>
                        @error('description') <p class="text-xs text-[#FF3B30] mt-1">{{ $message }}</p> @enderror
                    </div>
                ...
                <div class="px-6 py-4 bg-gray-50/90 border-t border-[#d2d2d7]/60 flex justify-end gap-3">
                    <button type="button" @click="open = false" class="bg-white border border-[#d2d2d7] text-[#1d1d1f] text-sm font-medium rounded-xl px-4 py-2 hover:bg-gray-100 cursor-pointer">
                        Cancelar
                    </button>
                    <button type="submit" class="bg-[#007AFF] text-white text-sm font-medium rounded-xl px-5 py-2 hover:bg-[#0056CC] cursor-pointer" x-text="isEdit ? 'Guardar cambios' : 'Crear colección'">
                    </button>
                </div>
            </form>
        </div>
